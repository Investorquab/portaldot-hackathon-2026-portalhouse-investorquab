...
export function useChain() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // Subscribe to new heads — deduplicated
      const unsubHeads = await newApi.rpc.chain.subscribeNewHeads(async (header) => {
        ...
        setRecentBlocks(prev => {
          const next = [{ number: blockNum, hash: blockHash, time: Date.now() }, ...prev]
        ...
        try {
          ...
          block.block.extrinsics.forEach((ex, i) => {
            ...
            setRecentTxs(prev => {
              const next = [{ id: txId, block: blockNum, hash: blockHash, method, call, signer, time: Date.now() }, ...prev]
  ...
  const getBalance = useCallback(async (address) => {
    ...
    try {
      ...
      return {
        free: formatBalance(data.free, { forceUnit: '-' }),
        reserved: formatBalance(data.reserved, { forceUnit: '-' }),
        raw: data.free,
      }
    } catch { return null }
  }, [])
  ...
  // Universal signer — tries all extensions
  const getSigner = useCallback(async (address) => {
    ...
    throw new Error('No signer found for this address. Make sure your wallet is connected.')
  }, [])
